...
function TweetsList({ userId, onRefresh, showGlobalFeed = false }) {
  ...
  const loadTweets = async () => {
    try {
      ...
      if (showGlobalFeed) {
        ...
        fetchedTweets = await getAllTweets()
      } else {
        // Fetch only user tweets for profile view
        fetchedTweets = await getUserTweets(targetUserId)
      }
  ...
  useEffect(() => {
    loadTweets()
  }, [targetUserId, onRefresh, showGlobalFeed])
